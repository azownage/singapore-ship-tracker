...
# Load maritime zones (Anchorages, Channels, Fairways)
@st.cache_data
...
# S&P Maritime API - Full v3.71 Implementation
class SPMaritimeAPI:
    ...
    def get_ship_risk_data(self, imo_numbers: List[str]) -> Dict[str, Dict]:
        """Get complete S&P v3.71 compliance screening"""
        if not imo_numbers:
            return {}
                    ...
                    if 'ShipResult' in data:
                        for ship in data['ShipResult']:
                            if 'APSShipDetail' in ship:
                                ...
                                cache[imo] = self._extract_complete_compliance(detail)
    ...
    def _extract_complete_compliance(self, detail: Dict) -> Dict:
        """Extract complete S&P v3.71 compliance screening"""
        ...
        # Calculate Legal Overall (highest value)
        all_scores = [ship_bes, ship_eu, ship_ofac, ship_un, port_call_3m, 
                     dark_activity, flag_disputed, owner_un, owner_ofac]
...
class AISTracker:
    ...
    def _create_vessel_polygon(self, row):
        ...
        try:
            ...
            # Use larger defaults for visibility
            ...
            corners = [
                (-dim_c, dim_a),   # Port bow
                (dim_d, dim_a),    # Starboard bow
                (dim_d, -dim_b),   # Starboard stern
                (-dim_c, -dim_b),  # Port stern
            ]
